...
SIM = (int(sys.argv[6]) if len(sys.argv)>=7 else 11)
sim_idx = str(SIM).zfill(9)
SEED = (int(sys.argv[7]) if len(sys.argv)>=8 else 0)
random.seed(SEED)
np.random.seed(SEED)
...
def calc_hazard(lmbda,gamma,conflict_at_t,discounted_conflict,delt,function_type='constant'):
    ...
    elif function_type in ['constant_conflict','constant_global_conflict','constant_conflict_intensity','constant_global_conflict_intensity','constant_global_conflict_lagged_intensity']:
        return purifier(lmbda*(1-conflict_at_t))
    elif function_type=='constant_global_conflict_weighted_intensity':
        return purifier(lmbda*(1-purifier(gamma*conflict_at_t,abs_min=0,abs_max=1)))
