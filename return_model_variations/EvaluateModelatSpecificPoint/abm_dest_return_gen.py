...
if function_type in ['constant','constant_conflict','constant_global_conflict','constant_global_conflict_intensity','constant_conflict_intensity']:
    for r in REGION_BIN_DICT:
        for ss in good_sims:
            print('sbatch -J',job_name,'abm_dest_return.sbatch',calibration_iteration,r,function_type+'#'+dir_prefix,H_rate,0.01,1.0,3.0,ss)
...
if 'split' in function_type:
    V_rate = float(sys.argv[6])
    for r in REGION_BIN_DICT:
        for ss in good_sims:
            print('sbatch -J',job_name,'abm_dest_return.sbatch',calibration_iteration,r,function_type+'#'+dir_prefix,H_rate,V_rate,1.0,3.0,ss)
